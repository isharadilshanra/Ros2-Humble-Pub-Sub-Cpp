...
: rclcpp::Node("my_subscriber") // Initializes a MySubscriber object
                                // It initializes the base class rclcpp::Node with the name "my_subscriber"
{  
    p_Subscription = this->create_subscription<std_msgs::msg::String>("my_topic", // Creates a subscription to "my_topic" with a message type of std_msgs::msg::String
                                                                        10,       // The subscription will have a queue size of 10
                                                                        std::bind(&MySubscriber::TopicCallback, // callback function for handling received messages
...
{
    RCLCPP_INFO(this->get_logger(), "I heard: '%s'", msg.data.c_str()); // ROS2 logging
}
